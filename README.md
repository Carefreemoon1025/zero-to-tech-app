...
第一版首屏的间距是"看着差不多"定的，结果标题区发闷。量了一下才发现问题：
小标题离标题只有 14px、标题离副标题 16px，而标题本身有 54px 高；上内边距 28px、左右却 44px
——**上方挤、左右松**，读起来就是"拥挤"。

现在按"大标题四周至少留出它半个身高的空隙"重排：

| 位置 | 之前 | 现在 |
|---|---|---|
| 面板内边距 | 28 / 44 / 42 | 64 / 52 / 56 |
| 小标题 → 标题 | 14px | 22px |
| 标题 → 副标题 | 16px | 24px |
| 副标题 → 下滑提示 | 30px | 42px |
| 标题字号 | clamp(30, 4.2vw, 54) | clamp(30, 3.7vw, 50) |

首屏还从**左对齐改成了居中**：面板有 980px 宽，而这一屏只有一句欢迎语，
左对齐会让右边空出一大片、内容全挤在左边；居中之后左右留白是对称的。
（内容区的标题仍然左对齐——正文左对齐更好读，居中只用在首屏这一句上。）

两个页面的标题也统一了尺度：文字实验室的标题原来是 96px 封顶，比首页的欢迎语还大一倍，
现在都是"页面级标题"的同一档（clamp(32, 4vw, 56)）；它那两张卡也从纯白改成磨砂玻璃，
和首页同一套语言，但更"实"一点——里面是输入框和按钮，可读性与可点性优先。
